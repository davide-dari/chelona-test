...
await clickExact('Casa'); await new Promise(r => setTimeout(r, 800));
await clickExact('Volantino'); await new Promise(r => setTimeout(r, 5000));
console.log('--- TEXT ---');
console.log((await page.evaluate(() => document.body.innerText)).slice(0, 700));
console.log('--- BTNS ---');
console.log(JSON.stringify(await page.evaluate(() => [...document.querySelectorAll('button')].map(b => (b.textContent||'').trim().slice(0,30)).filter(Boolean)).catch(()=>[])));
