...
const target = inp.find(i => /nome/i.test(i.ph || ''));
if (target) {
  const idx = inp.indexOf(target);
  await page.evaluate((idx) => { const i = document.querySelectorAll('input')[idx]; const s = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set; s.call(i, 'Test'); i.dispatchEvent(new Event('input', { bubbles: true })); }, idx);
}
await new Promise(r => setTimeout(r, 400));
await clickText('Crea Profilo'); await new Promise(r => setTimeout(r, 2500));
