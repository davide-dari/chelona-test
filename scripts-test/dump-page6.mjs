...
await clickText('Crea Profilo');
await new Promise(r => setTimeout(r, 2000));
console.log((await page.evaluate(() => document.body.innerText)).slice(0, 700));
console.log('--- BTNS ---', JSON.stringify(await page.evaluate(() => [...document.querySelectorAll('button')].map(b => (b.textContent||'').trim().slice(0,25)).filter(Boolean)).catch(()=>[])));
await browser.close();
