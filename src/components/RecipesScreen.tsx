...
interface MealDetail {
  ...
  strTags: string;
  strYoutube: string;
  [key: string]: any; // for dynamic ingredient/measure keys
}
...
export function RecipesScreen({ onClose }: RecipeScreenProps) {
  ...
  const filteredAreas = areas.filter(a => a.strArea.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {filteredAreas.map((area, idx) => (
                ...
                >
                  ...
                  <span className="font-bold text-[var(--text-main)] text-sm md:text-base">{area.strArea}</span>
                </motion.button>
              ))}
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold text-[var(--text-main)] flex items-center gap-2">
                  <span className="text-orange-500">{selectedArea}</span> Cuisine
                </h2>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {meals.map((meal, idx) => (
                  <motion.div
                    ...
                    className="bg-[var(--card-bg)] border border-[var(--border)] rounded-3xl overflow-hidden cursor-pointer hover:shadow-xl hover:shadow-orange-500/10 hover:-translate-y-1 transition-all group"
                  >
